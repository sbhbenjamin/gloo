...
describe("Jane Favourites contains favourited products", () => {
  it("Go to favourites page", () => {
    ...
    cy.wait(5000) // wait for both products to load
  })
  ...
  it("Unfavourite products", () => {
    ...
    cy.contains("SZ Painting").should("exist")
    cy.wait(5000) //when the first object is unfavourited, SZ Painting becomes the only item in favourites and moves to the left. without this wait statement, cypress will click on the original location of the heart, which is now nothing
    cy.contains("SZ Painting")
      .parent("div")
      .within(() => {
        cy.get("[class='fas fa-heart fa-lg']").should("exist").click()
