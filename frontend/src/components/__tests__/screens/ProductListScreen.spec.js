...
it("should not render products when logged in", async () => {
  const match = {
    params: {
      keyword: "",
      pageNumber: 1,
  ...
  render(<ProductListScreen history={history} match={match} />)
  expect(screen.getByRole("alert")).toHaveTextContent(
    "You need to be logged in as an Admin to view this page"
  )
})

it("should not render products when logged in as regular user", async () => {
  const match = {
    params: {
      keyword: "",
      pageNumber: 1,
