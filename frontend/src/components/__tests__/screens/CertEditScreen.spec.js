...
it("CertEditScreen with details should render if logged in as admin", async () => {
  ...
  await waitFor(() => {
    ...
    expect(screen.queryByTestId("approve-cert")).toBeInTheDocument()
    expect(screen.queryByTestId("reject-cert")).toBeInTheDocument()
    ...
    fireEvent.click(screen.getByTestId("reject-cert"))
    fireEvent.click(screen.getByTestId("edit-submit"))
  })
})
...
it("should delete certificate successfully if logged in as owner of certificate", async () => {
  ...
  //   expect(history.location.pathname).toBe("/certificates")
