...
const Product = ({ product }) => {
  ...
  const getFavourite = useSelector((state) => state.getFavourite)
  const { product: favProduct } = getFavourite
  ...
  useEffect(() => {
    if (userInfo) {
      dispatch(getFavourite(userInfo._id, product._id))
    }
  }, [dispatch, userInfo, successAdd, successRemove, getFavourite, product])
